...
**Backend настроен на автоматическую перезагрузку!**

После запуска `docker-compose up -d`:
- ✅ Изменения в коде **автоматически** подхватываются
- ✅ **НЕ НУЖНО** пересобирать контейнер
- ✅ Uvicorn перезагружается за ~1-2 секунды
- ✅ Просто сохраните файл - изменения применятся

**Как это работает:**
1. Код монтируется как volume: `.:/app`
2. Uvicorn запущен с флагом `--reload`
3. При сохранении файла сервер автоматически перезапускается

**Когда НУЖНА пересборка:**
- ⚠️ Изменили `requirements.txt` (добавили зависимости)
- ⚠️ Изменили `Dockerfile`
- ⚠️ Изменили переменные окружения в `docker-compose.yaml`
